...
(function () {
  ...
  const C = {
    headerBg: "#f0f4f8",
    // unified light blue (title bar + nav)
    text: "#000",
    // black text (title + tabs)
    accent: "#c8102e",
    // brighter red active-page indicator
    faintLine: "rgba(0,0,0,0.15)",
    // faint black line under the header
    rule: "#d8dde3",
    // thin gray rule under the page title
    noticeBorder: "#c2ccd6",
    // slightly darker edge on notice cards
    noteText: "#6b7280",
    // gray note beside a section title
    link: "#1a5fb4",
    // blue action links
    courseLink: "#1a73e8",
    // brighter blue for course names
    btnBlue: "#1a73e8",
    // bright blue action buttons
    btnBlueHover: "#1666d0",
    fieldBorder: "#ced4da",
    // thin gray edge on inputs
    facetBg: "#f0f4f8",
    // light gray of an open filter block
    optDot: "#c2ccd6",
    // unchecked option circle
    gridLine: "#e9edf1",
    // hairline between rows
    edgeLine: "#ccd4dc",
    // edges of the scrolling pane
    groupLine: "#b8c2cc",
    // divider between course names: soft, thin
    zebraEven: "#ffffff",
    // zebra, fixed at load
    zebraOdd: "#f8fafc",
    // a touch more gray, still light
    rowFolded: "#d3e4fb",
    // a collapsed leader: clearly blue
    rowHover: "#eff5fe",
    // hover: lighter than the folded blue
    warnClash: "#fdecec",
    // time clash with an already-taken course
    warnClashText: "#a4121b",
    warnCredit: "#fff6e5",
    // would push the credit total over
    errorBorder: "#e5b4b4",
    // red edge on an error notice card
    successFill: "#e8f6ed",
    // light green fill of a success card
    successBorder: "#b5d8bf",
    // green edge of a success card
    headBg: "#f0f4f8"
    // grid header, same light blue as elsewhere
  };
  ...
  function buildHeader() {
    ...
    console.log(
      "[Beautiful PKU Elective] header built;",
      nav.children.length,
      "links; active =",
  ...
  function buildHero() {
    ...
    let meta = "";
    if (bracketMatch) {
      meta = bracketMatch[0].replace(/^[【\[]/, "").replace(/[】\]]$/, "").trim();
    }
  ...
  function collectNoticeCards(root, onlyOutcomes = false) {
    ...
    imgs.forEach((img) => {
      ...
      if (text && seenText.has(text)) {
        const outer2 = (_a = row.closest("table")) == null ? void 0 : _a.closest("tr");
        if (outer2) outers.add(outer2);
        else row.remove();
        return;
      }
      seenText.add(text);
      ...
      cards.push(card);
      const outer = (_b = row.closest("table")) == null ? void 0 : _b.closest("tr");
    ...
    toggle.addEventListener("click", () => {
      ...
      if (open) {
        ...
        panel.querySelectorAll(".pku-facet--open").forEach((f) => {
          ...
          (_a = f.querySelector(".pku-facet-btn")) == null ? void 0 : _a.setAttribute("aria-expanded", "false");
          (_b = f.querySelector(".pku-chev")) == null ? void 0 : _b.classList.remove("pku-chev--open");
        });
      }
    });
  ...
  let CREDIT_INFO = null;
  let LAST_OP_COURSE = null;
  ...
  function currentSearchQuery() {
    var _a;
    return (((_a = document.getElementById("pku-course-search")) == null ? void 0 : _a.value) || "").trim();
  }
  ...
  function captureRowValues(labels, r) {
    ...
    r.q = [txt(r.tr, iId), txt(r.tr, iName), txt(r.tr, iTeacher)].join(" ").toLowerCase();
    r.facets = {};
  ...
  const GRID_MODEL = /* @__PURE__ */ new WeakMap();
  ...
  function adoptForeignRow(tr, model, pageIndex) {
    ...
    if (paneFirst >= 0 && paneLast >= paneFirst) {
      ...
      if (group.length) {
        ...
        const ordered = paneOrder && paneOrder.length ? paneOrder.map((k) => group[k]) : group;
  ...
  function statusMatches(r, opt, taken) {
    switch (opt) {
      case "已满":
        return !!r.cap && r.cap.taken >= r.cap.limit;
      case "未满":
        return !!r.cap && r.cap.taken < r.cap.limit;
      case "冲突":
        return clashesWithTaken(r, taken);
      case "不冲突":
        return !clashesWithTaken(r, taken);
      default:
        return true;
  ...
  function applyFilter(grid, state) {
    ...
    model.rows.forEach((r) => {
      ...
      if (over) {
        why.push("学分超限：已选 " + fmtCredit(committed) + " + 本课 " + fmtCredit(r.credit) + " > " + fmtCredit(limit));
      }
  ...
  function collapseScrollColumns(grid, headRow, bodyRows, measured) {
    ...
    collapseScrollColumns.last = {
      ...
      em: paneEm.slice(),
      base: paneEm.slice(),
      // what was measured; `em` is re-derived from it
      at: first,
      span,
  ...
  function assignColumnWidths(grid, headRow, measured, pane) {
    ...
    cols.forEach((c) => {
      ...
      if (c.hold == 1 && c.w < COL_NARROW) {
        c.hold = SHORT_FIELD_MULTIPLY;
      }
      const W = c.w;
  ...
  function animateRows(rows, folded, onDone) {
    ...
    void rows[0].tr.getBoundingClientRect().height;
    requestAnimationFrame(() => {
      rows.forEach((r) => {
  ...
  function wireActions() {
    ...
    document.addEventListener("click", (e) => {
      const a = e.target && e.target.closest ? e.target.closest('a[href*="cancelCourse.do"]') : null;
